...
class TProcessor {
 public:
  ...
  virtual bool process(std::auto_ptr<protocol::TProtocol> in,
                       std::auto_ptr<protocol::TProtocol> out,
                       void* connectionContext) = 0;

  bool process(std::auto_ptr<apache::thrift::protocol::TProtocol> io,
               void* connectionContext) {
    return process(io, io, connectionContext);
  }

  std::auto_ptr<TProcessorEventHandler> getEventHandler() {
    return eventHandler_;
  }

  void setEventHandler(std::auto_ptr<TProcessorEventHandler> eventHandler) {
 ...
 protected:
  ...
  std::auto_ptr<TProcessorEventHandler> eventHandler_;
};

/**
 * This is a helper class to allow std::auto_ptr to be used with handler
...
class ReleaseHandler {
 public:
   ReleaseHandler(const std::auto_ptr<HandlerFactory_>& handlerFactory) :
 ...
 private:
   std::auto_ptr<HandlerFactory_> handlerFactory_;
};

struct TConnectionInfo {
  // The input and output protocols
  std::auto_ptr<protocol::TProtocol> input;
  std::auto_ptr<protocol::TProtocol> output;
  ...
  // output protocols.
  std::auto_ptr<transport::TTransport> transport;
};
...
class TProcessorFactory {
 public:
   ...
   */
  virtual std::auto_ptr<TProcessor> getProcessor(
      const TConnectionInfo& connInfo) = 0;
};

class TSingletonProcessorFactory : public TProcessorFactory {
 public:
  TSingletonProcessorFactory(std::auto_ptr<TProcessor> processor) :
      processor_(processor) {}

  std::auto_ptr<TProcessor> getProcessor(const TConnectionInfo&) {
    return processor_;
  }

 private:
  std::auto_ptr<TProcessor> processor_;
};
