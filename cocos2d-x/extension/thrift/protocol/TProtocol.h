...
class TProtocol {
 public:
  ...
  inline TTransport* getTransport() {
    return ptrans_;
  }

  // TODO: remove these two calls, they are for backwards
  // compatibility
  inline TTransport* getInputTransport() {
    return ptrans_;
  }
  inline TTransport* getOutputTransport() {
    return ptrans_;
  }

 protected:
  TProtocol(TTransport* ptrans):
    ptrans_(ptrans) {
  }
...
{
public:
	virtual uint32_t read(protocol::TProtocol* iprot, cocos2d::CCValueBuilder* vbuilder) = 0;
	virtual uint32_t write(protocol::TProtocol* oprot, cocos2d::CCValue* value) = 0;
};
