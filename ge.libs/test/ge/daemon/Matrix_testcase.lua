...
if false then
    local a1 = class.define("test.a1",{ge.daemon.MatrixCommand})    
    function a1:execute()
        print("a1")
    end
...
if true then
    -- event
    local cls = class.define("test.o",{ge.daemon.MatrixObject})
    
    local f = class.define("test.f",{ge.daemon.MatrixCommand})
    function f:ctor(o)
        self.target = o
    end
    function f:execute()        
